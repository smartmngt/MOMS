...
# 배치 병렬 fetch(메모리 안전) + 진행률 콜백 + 종목수 제한
...
def get_universe(limit=None):
    """전체 KRX 종목 [(code, name), ...]. limit 있으면 앞에서 그만큼."""
...
def scan_daily(scan_start, scan_end,
    ...
    # 배치로 나눠 처리 → 동시에 메모리에 올라가는 DataFrame 수를 batch_size로 제한
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        for i in range(0, total, batch_size):
            chunk = codes[i:i + batch_size]
            futs = [ex.submit(_fetch_one, code, name, fetch_start, fetch_end)
